...
# NEW: default to ./recordings to match the capture script,
# but also scan LOG_DIR/videos for legacy files.
DEFAULT_PRIMARY_VIDEO_DIR = os.path.join(os.getcwd(), "recordings")
ENV_VIDEO_DIR = os.environ.get("GESTURE_VIDEO_DIR", "").strip() or None
LEGACY_VIDEO_DIR = os.path.join(LOG_DIR, "videos")

# Ordered search paths (first = preferred for new files)
VIDEO_DIRS: List[str] = []
for p in [ENV_VIDEO_DIR, DEFAULT_PRIMARY_VIDEO_DIR, LEGACY_VIDEO_DIR]:
    if p and p not in VIDEO_DIRS:
        VIDEO_DIRS.append(p)

app = FastAPI(title="Gesture Logs & Videos Service", version="0.3.2")
...
def _ensure_video_dirs() -> List[str]:
    ensured: List[str] = []
    for d in VIDEO_DIRS:
...
def _iter_video_paths() -> List[str]:
    _ensure_video_dirs()
    files: List[str] = []
    for d in VIDEO_DIRS:
        files.extend(glob.glob(os.path.join(d, "*")))
    # unique while preserving order (first-seen wins)
    seen = set()
    uniq = []
    for p in files:
        if p not in seen:
...
def _safe_video_path(filename: str) -> Optional[str]:
    # prevent path traversal, then resolve by search order
    base = os.path.basename(filename)
    for d in VIDEO_DIRS:
        p = os.path.abspath(os.path.join(d, base))
        # ensure it's within the directory (no traversal)
...
def get_video(filename: str):
    ...
    return FileResponse(path, media_type=_guess_mime(filename))
